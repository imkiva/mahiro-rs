...
/// A cursor position in a `&str` which provides useful methods to manually
/// parse that string.
#[derive(Clone)]
pub struct Position<'i> {
  ...
  /// # Safety:
  ///
  /// `input[pos..]` must be a valid codepoint boundary (should not panic when
  /// indexing thus).
  pos: usize,
}
...
#[allow(dead_code)]
impl<'i> Position<'i> {
  /// Create a new `Position` without checking invariants. (Checked with
  /// `debug_assertions`.)
  ///
  /// # Safety:
  ///
  ...
  /// Attempts to create a new `Position` at the given position. If the
  /// specified position is an invalid index, or the specified position is not
  /// a valid UTF8 boundary, then None is returned.
  ///
  /// # Examples
  /// ```
  /// # use pest::Position;
  ...
  pub fn span(&self, other: &Position<'i>) -> span::Span<'i> {
    ...
    {
      // This is safe because the pos field of a Position should always be a valid str
      // index.
  ...
  /// Returns `true` when the `Position` points to the start of the input
  /// `&str`.
  #[inline]
  ...
  /// Skips `n` `char`s from the `Position` and returns `true` if the skip was
  /// possible or `false` otherwise. If the return value is `false`, `pos`
  /// will not be updated.
  #[inline]
  ...
  /// Goes back `n` `char`s from the `Position` and returns `true` if the skip
  /// was possible or `false` otherwise. If the return value is `false`, `pos`
  /// will not be updated.
  #[inline]
  ...
  /// Skips until one of the given `strings` is found. If none of the `strings`
  /// can be found, this function will return `false` but its `pos` will
  /// *still* be updated.
  #[inline]
  ...
  /// Matches the char at the `Position` against a filter function and returns
  /// `true` if a match was made. If no match was made, returns `false` and
  /// `pos` will not be updated.
  #[inline]
  ...
  /// Matches `string` from the `Position` and returns `true` if a match was
  /// made or `false` otherwise. If no match was made, `pos` will not be
  /// updated.
  #[inline]
  ...
  /// Case-insensitively matches `string` from the `Position` and returns `true`
  /// if a match was made or `false` otherwise. If no match was made, `pos`
  /// will not be updated.
  #[inline]
  ...
  /// Matches `char` `range` from the `Position` and returns `true` if a match
  /// was made or `false` otherwise. If no match was made, `pos` will not be
  /// updated.
  #[inline]
