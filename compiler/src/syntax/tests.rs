...
use crate::syntax::tree::Expr::{
    Alloc, Apply, Assign, Binary, Id, Lambda, Literal, Question, Ternary, Unary,
};
use crate::syntax::tree::Header::{Import, Package, Using};
use crate::syntax::tree::Ident;
use crate::syntax::tree::Lit::{Array, Bool, Char, Null, Number, Pair, Str};
...
use crate::syntax::tree::Stmt::{
    ...
    Throw, Try, Var, VarList, While,
};
use crate::syntax::tree::VarInit::{Simple, Structured};
use crate::syntax::tree::{Op, Program};
...
mod parse {
    ...
    fn parse_import() {
        ...
        assert_eq!(
            prog,
            vec![HeaderEntry(Import(Ident::only("streams"), None))]
        )
    }
    ...
    fn parse_import_dot() {
        ...
        assert_eq!(
            prog,
            vec![HeaderEntry(Import(Ident::only("codec.base64"), None))]
        )
    }
    ...
    fn parse_throw() {
        ...
        assert_eq!(prog, vec![StmtEntry(Throw(Id(Ident::only("boynextdoor"))))]);
    }
            ...
            vec![
                ...
                StmtEntry(Var(Simple(
                    ...
                    Unary(
                        Loc::Injected,
                        Op::Sub,
                        Box::new(Literal(Loc::Injected, Number(1000 as f64)))
                    )
                ))),
                StmtEntry(Var(Simple(
                    Ident::only("d"),
                    Unary(
                        Loc::Injected,
                        Op::Sub,
                        Box::new(Literal(Loc::Injected, Number(10086.123 as f64)))
                    )
                ))),
                StmtEntry(Var(Simple(
                    Ident::only("e"),
                ...
                StmtEntry(Var(Simple(
                    ...
                    Unary(
                        Loc::Injected,
                        Op::Add,
                        Box::new(Literal(Loc::Injected, Number(132 as f64)))
                    )
                ))),
                StmtEntry(Var(Simple(
                    Ident::only("g"),
                    Unary(
                        Loc::Injected,
                        Op::Add,
                        Box::new(Literal(Loc::Injected, Number(114514.1232)))
                    )
                ))),
                StmtEntry(Var(Simple(
                    Ident::only("h"),
                    Unary(
                        Loc::Injected,
                        Op::Add,
                        Box::new(Literal(Loc::Injected, Number(0 as f64)))
                    )
                ))),
                StmtEntry(Var(Simple(
                    Ident::only("i"),
    ...
    fn parse_bool_literal() {
        ...
        assert_eq!(
            ...
            vec![
                StmtEntry(Var(Simple(
                    Ident::only("a"),
                    Literal(Loc::Injected, Bool(true))
                ))),
                StmtEntry(Var(Simple(
                    Ident::only("b"),
                    Literal(Loc::Injected, Bool(false))
                ))),
            ]
        );
    }
    ...
    fn parse_string_literal() {
        let prog = parse("var a = \"boy next door\"");
        assert_eq!(
            prog,
            vec![StmtEntry(Var(Simple(
                Ident::only("a"),
                Literal(Loc::Injected, Str("boy next door".into()))
            ))),]
        );
    }
    ...
    fn parse_string_literal_with_escape() {
        let prog = parse("var a = \"boy\\nnext\\ndoor\"");
        assert_eq!(
            prog,
            vec![StmtEntry(Var(Simple(
                Ident::only("a"),
                Literal(Loc::Injected, Str("boy\nnext\ndoor".into()))
            ))),]
        );
    }
    ...
    fn parse_char_literal() {
        let prog = parse("var a = 'Z'");
        assert_eq!(
            prog,
            vec![StmtEntry(Var(Simple(
                Ident::only("a"),
                Literal(Loc::Injected, Char('Z'))
            ))),]
        );
    }
    ...
    fn parse_array_literal() {
        ...
        assert_eq!(
            ...
            vec![
                StmtEntry(Var(Simple(
                    Ident::only("a"),
                    Literal(Loc::Injected, Array(vec![]))
                ))),
                StmtEntry(Var(Simple(
                    Ident::only("b"),
                    Literal(
    ...
    fn parse_array_pair_literal() {
        let prog = parse("var a = {0: a, 1: b, 2: c}");
        assert_eq!(
            prog,
            vec![StmtEntry(Var(Simple(
                Ident::only("a"),
                Literal(
                    ...
                    Array(vec![
                        ...
                        Literal(
                            Loc::Injected,
                            Pair(
                                Box::new(Literal(Loc::Injected, Number(1 as f64))),
                        ...
                        Literal(
                            Loc::Injected,
                            Pair(
                                Box::new(Literal(Loc::Injected, Number(2 as f64))),
                    ...
                    Literal(
                        ...
                        Pair(
                            ...
                            Box::new(Lambda(
                                Loc::Injected,
                                None,
                                vec![Normal(Ident::only("a")), Normal(Ident::only("b")),],
                                Box::new(Binary(
                                    Loc::Injected,
                                    Op::Add,
                                    Box::new(Id(Ident::only("a"))),
    ...
    fn parse_lambda_with_varargs() {
        let prog = parse("var format = [](fmt, ...arg) -> echo(fmt, arg...)");
        assert_eq!(
            prog,
            vec![StmtEntry(Var(Simple(
                Ident::only("format"),
                Lambda(
                    Loc::Injected,
                    None,
                    vec![Normal(Ident::only("fmt")), Varargs(Ident::only("arg")),],
                    Box::new(Apply(
                        Loc::Injected,
    ...
    fn parse_lambda_with_primary_expr() {
        let prog = parse("var show = [](a) -> system.out.println(a)");
        assert_eq!(
            prog,
            vec![StmtEntry(Var(Simple(
                Ident::only("show"),
    ...
    fn parse_lambda_with_captures() {
        ...
        assert_eq!(
            ...
            vec![StmtEntry(Var(Simple(
                ...
                Lambda(
                    Loc::Injected,
                    Some(vec![Ident::only("a"), Ident::only("b"), Ident::only("c"),]),
                    vec![],
                    Box::new(Id(Ident::only("lam"))),
                )
            ))),]
    ...
    fn parse_expr_stmt() {
        ...
        assert_eq!(
            prog,
            vec![
                StmtEntry(ExprStmt(Apply(
                    Loc::Injected,
                    Box::new(Id(Ident::only("exit"))),
                    vec![]
                ))),
                StmtEntry(ExprStmt(Assign(
                    Loc::Injected,
                    Op::Assign,
                    Box::new(Id(Ident::only("a"))),
    ...
    fn parse_func_decl() {
        ...
        assert_eq!(
            prog,
            vec![StmtEntry(Func(Ident::only("jiegebuyao"), vec![], vec![]))]
        );
    }
    ...
    fn parse_empty_namespace_decl() {
        ...
        assert_eq!(
            prog,
            vec![StmtEntry(Namespace(Ident::only("jiegebuyao"), vec![],))]
        );
    }
    ...
    fn parse_struct_decl_with_extends() {
        ...
        assert_eq!(
            ...
            vec![StmtEntry(Struct(
                Ident::only("jiegebuyao"),
                Some(Id(Ident::only("boynextdoor"))),
                vec![
                    Var(Simple(Ident::only("awsl"), Id(Ident::only("jiegebuyao")))),
                    Var(Simple(
                        Ident::only("binbin"),
    ...
    fn parse_empty_block_decl() {
        ...
        assert_eq!(prog, vec![StmtEntry(Block(vec![],))]);
    }

    #[test]
    fn parse_return() {
        let prog = parse("return");
        assert_eq!(prog, vec![StmtEntry(Return(Loc::Injected, None))]);
    }
    ...
    fn parse_return_sth() {
        ...
        assert_eq!(
            prog,
            vec![StmtEntry(Return(
                Loc::Injected,
                Some(Id(Ident::only("i_single_push_minato_aqua")))
            ))]
        );
    }
    ...
    fn parse_if_else() {
        ...
        assert_eq!(
            ...
            vec![StmtEntry(If(
                ...
                Some(vec![ExprStmt(Apply(
                    Loc::Injected,
                    Box::new(Id(Ident::only("no"))),
                    vec![]
                )),])
            ))]
        );
    ...
    fn parse_continue() {
        ...
        assert_eq!(
            ...
            vec![StmtEntry(While(
                Literal(Loc::Injected, Bool(true)),
                vec![Continue(Ident::only("continue"))]
            ))]
        );
    }
    ...
    fn parse_for_each() {
        ...
        assert_eq!(
            prog,
            vec![StmtEntry(ForEach(
                Ident::only("i"),
                Apply(
                    Loc::Injected,
                    Box::new(Id(Ident::only("range"))),
                    vec![Literal(Loc::Injected, Number(10.0))]
                ),
                vec![ExprStmt(Apply(
                    Loc::Injected,
                    Box::new(Id(Ident::only("love"))),
                    vec![Id(Ident::only("i"))]
                ))],
            ))]
        );
        ...
        assert_eq!(
            ...
            vec![StmtEntry(Var(Simple(
                ...
                Alloc(
                    ...
                    vec![Id(Ident::only("awsl")), Id(Ident::only("ansl")),]
                ),
            )))]
        );
    }
    ...
    fn desugar_sign_before_literal() {
        ...
        assert_eq!(
            ...
            vec![
                StmtEntry(Var(Simple(
                    Ident::only("a"),
                    Literal(Loc::Injected, Number(-114.0))
                ))),
                StmtEntry(Var(Simple(
                    Ident::only("b"),
                    Literal(Loc::Injected, Number(514.0))
                ))),
            ]
        );
    }
...
mod optimize {
    ...
    impl LevelParse for OptimizeLevel {
        fn parse(self, input: &str) -> Program {
            Optimizer::run(super::desugar::parse(input), self).expect("Optimize Error")
        }
    }
