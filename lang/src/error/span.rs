...
/// A span over a `&str`. It is created from either [two `Position`s] or from a
/// [`Pair`].
///
...
#[allow(dead_code)]
impl<'i> Span<'i> {
  /// Create a new `Span` without checking invariants. (Checked with
  /// `debug_assertions`.)
  ///
  /// # Safety
  ///
  /// `input[start..end]` must be a valid subslice; that is, said indexing
  /// should not panic.
  ...
  /// Attempts to create a new span. Will return `None` if `input[start..end]`
  /// is an invalid index into `input`.
  ///
  /// # Examples
  ///
  /// ```
  /// # use pest::Span;
  /// let input = "Hello!";
  /// assert_eq!(None, crate::error::span::Span::new(input, 100, 0));
  /// assert!(crate::error::span::Span::new(input, 0, input.len()).is_some());
  /// ```
